...
jcmp.events.Add('C_race_end_point', function(player) {
  ...
  if (Race.type == 3){ // for just one line don't need to create a new checkpoint system and endpoint
  ...
  for (var i = 0; i < Race.leaderboard.length; i++) {
    ...
    if (player.networkId == playern) {
      ...
      Race.UpdateEndLeaderboard(playername, leaderboardplace, player.race.time);
      setTimeout(function() {
